...
RSpec.describe FileUploadsController, type: :request do
  ...
  before do
    ...
    allow(UploadedFileManager).to receive(:fetch_file_list).and_return([ uploaded_file ])
  ...
  describe "POST /file_uploads" do
    it "uploads a file successfully" do
      ...
      expect(JSON.parse(response.body)).to include("id", "name", "size", "line_count")
    end
  ...
  describe "DELETE /file_uploads/:id" do
    it "deletes a file successfully" do
      ...
      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)).to eq({ "message" => "File deleted successfully" })
    end
